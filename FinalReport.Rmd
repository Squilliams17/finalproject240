...
# Create a summary table using your correlation tests
correlation_table <- tibble(
  `X variable` = c(
    "2 FGM", "3 FGM", "2 FGM", "3 FGM"
  ),
  `Y variable` = c(
    "3 FGM", "WIN%", "WIN%", "LOSS%"
  ),
...
The graphs show that teams differ in their preference for 2-point versus 3-point shots, so these differences are visually noticeable. However, these patterns do not translate into meaningful correlations with playoff outcomes. Correlation tests confirm this: 2-point and 3-point field goals are weakly correlated with each other (r = -0.124, p = 0.230), and neither 2-point nor 3-point field goals significantly correlate with winning or losing percentages (all p-values > 0.6). Some variation in the data may be influenced by outlier teams, such as the Oklahoma City Thunder (OKC), who relied heavily on 2-point scoring while winning the championship, or the Los Angeles Lakers (LAL) and Miami Heat (MIA), who attempted a high volume of 3-pointers during shorter playoff runs. Despite these team-specific tendencies, the overall data and correlations indicate that no particular shooting style consistently predicts playoff success.


A correlation test suggests that the difference between 2-point and 3-point attempts is not statistically correlated with win percentage. Similarly, the correlation between a team’s 2-point field goal attempt percentage (2FGA%) and win percentage is also not statistically significant.
