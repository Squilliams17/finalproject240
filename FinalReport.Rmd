...
```{r, echo=FALSE}
shooting_data_table <- tibble(Columns = c("Player", "Restricted Area", "In the Paint (Non-RA)", "Mid-Range", "Left Corner 3", "Right Corner 3", "Corner 3", "Abover the Break 3", "2PT Field Goal", "3PT Field Goal"))
...
The team_data section looks at each team's performance in the playoffs, and maintains a team column with each Team's abbreviated name. The columns that are being used in this analysis are WIN%, which is the winning percentage of a team, directly correlating to their success in the playoffs. There are also aggregated fields to get values for 2 and 3 point shooting per game in their entirety for each team, structed the same as shooting_data classifies it per player.
...
In the following of this report, we will delve into the specifics of this data set, analyzing how 3-point shooting affects a given team's chances to win in the 2024-25 NBA Playoffs. The analysis will inquire about the proportion of 3 vs 2 point shots, and how they affect a team's winning percentage by constructing a linear regression model depending one these two variables to predict a team's winning percentage. In addition we will look at specific 3-point shooting data, and determine where the most effective areas of the court are for teams winning this year. We will look at the analysis of this data to determine whether 3-point shooting is as decisive of a factor towards winning in the NBA playoffs as popular media makes it out to be.
...
From the preceding figures, we can infer that 3-point shooting alone does not directly correlate with teams achieving a high winning percentage in the playoffs. The charts show that teams generally have higher win percentages when making more 2- and 3-point field goals, which makes sense as this directly translates to scoring more points throughout the game. There is definitely an appearance of teams doing well that average more 2s than the average. However, it appears there is little to no success for teams that shoot well below the average amount of 3's in a game. Some teams, such as the Boston Celtics (2023–24 NBA Champions), are outliers, maintaining a high winning percentage despite a relatively low average of 2-point field goals.

However, according to correlation tests, 2-point and 3-point field goals are weakly correlated with each other, and neither 2-point nor 3-point field goals significantly correlate with winning or losing percentages in the playoffs. Some variation may be influenced by outlier teams, such as the Oklahoma City Thunder (OKC), who relied heavily on 2-point scoring while winning the championship, or the Los Angeles Lakers (LAL) and Miami Heat (MIA), who attempted a high volume of 3-pointers during shorter playoff runs. 

A correlation test also shows that the difference between 2-point and 3-point attempts is not statistically correlated with win percentage, and similarly, a team’s 2-point field goal attempt percentage (2FGA%) does not significantly correlate with playoff outcomes.
...
correlation_table <- tibble(
  `X variable` = c(
    "Above-the-break 3 FG%","Left-corner 3 FG%","Right-corner 3 FG%",
    "2-point FG made","3-point FG made","2-point FG made",
    "2-point FG made","3-point FG made","2-pt minus 3-pt FGA","2-point FG%"
  ),
  `Y variable` = c(
    "WIN%","WIN%","WIN%","WIN%","WIN%","3-point FGM",
    "LOSS%","LOSS%","WIN%","WIN%"
  ),
...
```
These results indicate that above-the-break three-point shooting efficiency is the only measure among those tested that shows a statistically significant correlation with playoff win percentage. The other shooting zones and raw field goal volumes (2-point or 3-point) are not statistically relevant for predicting playoff success. Additionally, the non-statistically significant correlation between 2-point and 3-point FGM with win percentage suggests that while teams may favor one type of shot over another, this preference alone does not strongly predict playoff outcomes. This conclusion is further supported by a regression-based hypothesis test, which found no significant difference between the effects of 2FGM and 3FGM on win percentage. Overall, these findings suggest that only above-the-break three-point shooting demonstrates a statistically significant relationship with playoff success, while all other measures—including 2-point makes and 3-point makes or attempts—do not.

Overall, these findings partially support our initial hypothesis: while we expected three-point efficiency in general to be a stronger predictor of playoff success, only above-the-break three-point shooting shows a statistically significant relationship, and other measures—including 2-point makes and overall 3-point makes or attempts—do not.
